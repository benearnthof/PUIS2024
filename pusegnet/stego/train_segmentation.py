...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        if cfg.arch == "feature-pyramid":
            ...
            self.net = DinoFeaturizer(dim, cfg) # are there any tangible differences here?
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.cfg.correspondence_weight > 0:
            ...
            self.log('loss/pos_intra', pos_intra_loss.detach().cpu(), **log_args)
            self.log('loss/pos_inter', pos_inter_loss.detach().cpu(), **log_args)
            self.log('loss/neg_inter', neg_inter_loss.detach().cpu(), **log_args)
            self.log('cd/pos_intra', pos_intra_cd.mean().detach().cpu(), **log_args)
            self.log('cd/pos_inter', pos_inter_cd.mean().detach().cpu(), **log_args)
            self.log('cd/neg_inter', neg_inter_cd.mean().detach().cpu(), **log_args)
        ...
        if self.cfg.rec_weight > 0:
            ...
            self.log('loss/rec', rec_loss.detach().cpu(), **log_args)
        ...
        if self.cfg.aug_alignment_weight > 0:
            ...
            self.log('loss/aug_alignment', aug_alignment.detach().cpu(), **log_args)
        ...
        if self.cfg.crf_weight > 0:
            ...
            self.log('loss/crf', crf.detach().cpu(), **log_args)
        ...
        loss += linear_loss
        self.log('loss/linear', linear_loss.detach().cpu(), **log_args)

        cluster_loss, cluster_probs = self.cluster_probe(detached_code, None)
        loss += cluster_loss
        self.log('loss/cluster', cluster_loss.detach().cpu(), **log_args)
        self.log('loss/total', loss.detach().cpu(), **log_args)
...
# TODO: add a way to avoid hardcoding config
@hydra.main(config_path="/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ru25jan4/gitroot/stegorep/stegorep/src/configs", config_name="cityscapes.yaml")
def my_app(cfg: DictConfig) -> None:
    ...
    val_dataset = ContrastiveSegDataset(
        ...
        crop_type=None,
        image_set="val",
        transform=get_transform(cfg.val_res, False, val_loader_crop),
        target_transform=get_transform(cfg.val_res, True, val_loader_crop),
        mask=True,
        cfg=cfg,
    )
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                ...
                save_last=True,
                filename="step{step}-clust_acc{test/cluster/Accuracy:.2f}",
