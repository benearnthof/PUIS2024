...
import pusegnet.stego.dino.vision_transformer as vits
from pusegnet.stego.mxm_utils import only_dino
...
class DinoV2Featurizer(nn.Module):
    ...
    def make_nonlinear_clusterer(self, in_channels):
        # Try with more projection steps in between. 
        return torch.nn.Sequential(
            torch.nn.Conv2d(in_channels, in_channels, (1, 1)),
            torch.nn.ReLU(),
...
class ClusterLookup(nn.Module):
    """
    This is the cluster probe we will use to obtain pseudo labels for the SNGP segmentation head.
    """
